...
interface ExchangesClientContainerProps {
  initialExchanges: Exchange[];
  initialOrders: Order[];
  initialProducts: Product[];
}

export default function ExchangesClientContainer({
  initialExchanges,
  initialOrders,
  initialProducts,
}: ExchangesClientContainerProps) {
  return (
    <App>
      <AdminExchanges
...
function AdminExchanges({
  ...
  const { data: exchanges = initialExchanges, isLoading: isLoadingExchanges } =
    useAllExchanges();
  const { data: orders = initialOrders, isLoading: isLoadingOrders } =
    useAllOrders();
  ...
  const handleApprove = async (exchange: Exchange) => {
    try {
      await updateExchangeStatus.mutateAsync({
        id: exchange.id,
        status: "processing",
      });
    ...
    },
    {
      ...
      render: (_: unknown, r: Exchange) => (
        <div>
          ...
          {r.reasonDetail && (
            <p className="text-[10px] text-gray-500">{r.reasonDetail}</p>
          )}
        </div>
      ),
    },
        ...
        <Space>
          {r.status === "requested" && (
            <Button
              ...
              type="primary"
              loading={
                updateExchangeStatus.isPending || executeAction.isPending
              }
  ...
  const isLoadingAll =
    isLoadingExchanges || isLoadingOrders || isLoadingProducts;

  if (isLoadingAll && exchanges.length === 0) {
    return (
  ...
  const filteredExchanges = exchanges.filter((exc) => {
    ...
    const item = relatedOrder?.items?.[exc.orderItemIndex];
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-[#7E8299]">
            총 {filteredExchanges.length}건의 요청이 있습니다.
          </p>
        </div>
      </div>
