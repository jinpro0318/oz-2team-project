...
interface OrdersClientContainerProps {
  initialOrders: Order[];
}

export default function OrdersClientContainer({
  initialOrders,
}: OrdersClientContainerProps) {
  const { message, modal } = App.useApp();
  const [activeTab, setActiveTab] = useState("all");
  const [drawerOrder, setDrawerOrder] = useState<Order | null>(null);
  const [orders, setOrders] = useState<Order[]>(initialOrders);
  const [isOrdersLoading, setIsOrdersLoading] = useState(false); // 서버에서 데이터를 받아오므로 초기값 false
  const [selectedCelebId, setSelectedCelebId] = useState<string>("");

  // 실시간 동기화 (최초 렌더링 이후에 클라이언트에서 Firestore 구독)
  useEffect(() => {
    ...
    return orders.filter((o) => {
      const matchesCeleb =
        o.items?.some((item) => {
          ...
          return (
            prod?.celebrityId === selectedCelebId ||
            item.product?.celebrityId === selectedCelebId
          );
        }) ?? false;
      return matchesCeleb;
    });
  ...
  const handleRejectClaim = async (order: Order) => {
    ...
    try {
      await executeAction.mutateAsync({
        ...
        extraData: {
          reason: `판매자가 클레임을 반려했습니다. 사유: ${reason}`,
        },
      } as any);
      message.warning(
  ...
  const handleRejectCancel = async (order: Order) => {
    ...
    try {
      await executeAction.mutateAsync({
        ...
        extraData: {
          reason: `판매자가 취소 요청을 거절했습니다. 사유: ${reason}`,
        },
      } as any);

      message.warning(
        "취소 요청이 거절되어 '상품 준비중' 상태로 복귀했습니다.",
      );
      if (drawerOrder?.id === order.id) {
        setDrawerOrder({ ...order, status: "preparing" });
      }
            ...
            <div className="flex items-center gap-1">
              ...
              <div className="flex items-center gap-1.5">
                <Select
                  ...
                  onChange={async (val) => {
                    try {
                      const { CodeFulfillmentEngine } = await import(
                        "@/lib/services/CodeFulfillmentEngine"
                      );
                          ...
                          content: (
                            <div className="py-4 text-center">
                              ...
                              <p className="text-sm text-gray-500">
                                CODE 로지스틱스 시뮬레이션 송장이 임시 할당되었습니다.
                                <br />
                                오른쪽의 <b className="text-[#181C32]">['준비중']</b>{" "}
                    ...
                    modal.confirm({
                      ...
                      content: (
                        <div className="text-center">
                          설정된 택배사 및 송장번호 정보를 모두 제거하시겠습니까?
                        </div>
                      ),
                      okText: "삭제",
                      cancelText: "취소",
  ...
  return (
    <div>
      ...
      <AdminCelebFilterStrip
        ...
        dbSettingsKey="admin_orders_filter"
      />
          ...
          <div className="flex items-center gap-2">
            ...
            {drawerOrder &&
              (() => {
                ...
                return (
                  <Tag
                    color={statusConfig[drawerOrder.status]?.color ?? "default"}
                  >
