import { MOCK_STANDARD_PATH, MOCK_EXCHANGE_PATH, MOCK_RETURN_PATH, PathStep } from "./logistics";
...
export class LogisticsStatusResolver {
   ...
   */
  static getTargetIndex(orderStatus: string, shipmentType: "S" | "R" | "E"): number {
    if (shipmentType === "S") {
   ...
   */
  static getUISteps(shipmentType: "S" | "R" | "E" | "none" | null): { title: string }[] {
    if (shipmentType === "R") {
   ...
   */
  static getShipmentStatusForIndex(index: number, shipmentType: "S" | "R" | "E"): string {
  ...
  static resolveAction(
    ...
    switch (intent) {
      ...
      case "DISPATCH":
        if (shipmentType === "E") return { status: "reshipping", step: 4, shouldUpdateShipment: true };
        return { status: "shipping", step: 2, shouldUpdateShipment: true };
      case "DELIVER":
        if (shipmentType === "E") return { status: "exchange_completed", step: 5, shouldUpdateShipment: true };
        if (shipmentType === "R") return { status: "return_completed", step: 3, shouldUpdateShipment: true };
      ...
      case "EXCHANGE_DONE":
        return { status: "exchange_completed", step: 5, shouldUpdateShipment: true };
      case "CLAIM_REJECT":
      ...
      case "SIMULATE_NEXT": {
        ...
        return { status: nextStatus, step: nextStep, shouldUpdateShipment: !!nextStatus };
      }

      case "REVERT_PHASE": {
        const prevStep = Math.max(0, currentStep - 1);
        const prevStatus = this.getStatusFromIndex(prevStep, shipmentType);
        return { status: prevStatus, step: prevStep, shouldUpdateShipment: !!prevStatus };
      }

      default:
        throw new Error(`[LogisticsStatusResolver] 알 수 없는 명령어(Intent)입니다: ${intent}`);
    }
  }
   ...
   */
  static getStatusFromIndex(index: number, shipmentType: "S" | "R" | "E"): string | undefined {
